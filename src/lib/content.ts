...
const projectsEn: Project[] = [
  ...
  },
  {
    ...
    tags: ["Next.js 16", "TypeScript", "WordPress", "next-intl", "Railway", "Cloudflare"],
    desc: "Headless Next.js storefront on top of their existing WordPress catalog, serving property listings in English, Spanish, French, and German — where routing and SEO are the architecture, not an afterthought.",
    problem: "In real estate, nobody buys a house from a page that doesn't rank. Organic search is the whole game, and it's brutally unforgiving of exactly what a four-language site gets wrong: duplicate content across locales, canonical tags pointing at the wrong language, soft-404s eating crawl budget on deleted listings. The client's existing WordPress backend was built for a single-language brochure, not a headless multilingual storefront.",
    build: "A headless Next.js frontend on top of the agency's familiar WordPress catalog — but routing and SEO built as the architecture, not an afterthought.",
    points: [
      "One routing map for four locales — every translated slug defined once, with canonicals, hreflang tags, sitemaps, and nav links all reading from it. No copy-pasted route trees to drift out of sync.",
      "Computed canonicals and hreflang locally instead of trusting the CMS — WordPress's own canonical field silently returned the English URL for every locale, a bug that would have told Google to ignore the Spanish, French, and German pages.",
...
const projectsEs: Project[] = [
  ...
  },
  {
    id: "c21-perdomo", name: "C21 Perdomo", year: "2026", category: "Web app", cats: ["Web"],
    tags: ["Next.js 16", "TypeScript", "WordPress", "next-intl", "Railway", "Cloudflare"],
    desc: "Vitrina en Next.js (sin CMS acoplado) sobre el catálogo WordPress existente de Century 21, sirviendo propiedades en español, inglés, francés y alemán — donde el enrutamiento y el SEO son la arquitectura, no una idea tardía.",
    problem: "En bienes raíces, nadie compra una casa desde una página que no aparece en búsquedas. El SEO orgánico es todo el juego, y es brutalmente implacable con lo que falla en un sitio de cuatro idiomas: contenido duplicado entre locales, etiquetas canónicas apuntando al idioma equivocado, soft-404s devorando el presupuesto de rastreo. El WordPress del cliente estaba construido para un folleto monolingüe, no para una vitrina multilingüe sin CMS acoplado.",
    build: "Un frontend desacoplado en Next.js sobre el catálogo WordPress que el equipo ya conocía — pero con el enrutamiento y el SEO construidos como arquitectura, no como idea tardía.",
    points: [
      "Un solo mapa de rutas para cuatro idiomas — cada slug traducido se define una vez, y canónicas, hreflang, sitemaps y navegación leen de él. Sin árboles de rutas copiados y pegados que se desincronizan.",
      "Canónicas y hreflang calculadas localmente en vez de confiar en el CMS — el campo canónico de WordPress devolvía silenciosamente la URL en inglés para cada idioma, un bug que le habría dicho a Google que ignore las páginas en español, francés y alemán.",
  ...
  },
  {
    ...
    desc: "Una app que te frena antes de gastar de más — motor de IA de 3 compuertas, escaneo OCR de recibos, offline-first. Construida para el mercado dominicano.",
...
const es: typeof en = {
  ...
  home: {
    ...
    heroTitle1: "No hagas scroll.", heroTitle2: "Pregúntale al sitio.",
  ...
  about: {
    ...
    p1: "Desde Santiago, República Dominicana. Entrego integraciones de IA, automatizaciones y apps web y móviles completas — calidad de producción, rápido, sin la carga de una agencia. Ocho proyectos entregados este año, todos en vivo con usuarios reales.",
